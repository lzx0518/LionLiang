...
- 化学 1151 项引擎断言 + 34 个教材实验的逐步可行性校验；
  物理 **1506 项**断言（含 24 个实验每一步的逐步可达性、三种画布尺寸下的绘制越界检查、
  362 组「控制项 × 画布尺寸」穷举、以及指针扫格验证拖拽命中）；
...
└── test/
    ...
    ├── validate-physics.mjs   物理仿真 / 实验校验（含逐步可达性与绘制越界检查）
...
node test/validate-physics.mjs   # 物理：仿真运行 / 实验逐步可达 / 绘制越界
